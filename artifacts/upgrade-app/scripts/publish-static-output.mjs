...
function copyWithCollisionCheck(source, destination) {
  if (existsSync(destination)) {
    if (digest(source) !== digest(destination)) {
      fail(`Asset collision: ${relative(temporaryDirectory, destination)}`);
    }
    return;
  }
...
function validateOutput() {
  ...
  for (const [asset, expectedDigest] of Object.entries(historicalAssetDigests)) {
    assertFile(temporaryDirectory, asset);
    if (digest(join(temporaryDirectory, asset)) !== expectedDigest) {
  ...
  for (const document of prerenderedDocuments) {
    if (digest(join(temporaryDirectory, document)) === legacyDigest) {
      fail(`Prerendered document replaced by SPA fallback: ${document}`);
    }
  }

  for (const htmlFile of walkFiles(temporaryDirectory).filter((file) =>
    file.endsWith(".html"),
  )) {
    ...
    for (const reference of references) {
      assertFile(temporaryDirectory, reference.slice(1));
    }
  }
}
...
function promoteOutput() {
  ...
  if (existsSync(publicDirectory)) renameSync(publicDirectory, backupDirectory);
  ...
  try {
    ...
    if (existsSync(backupDirectory)) renameSync(backupDirectory, publicDirectory);
    throw error;
  }
}

if (!existsSync(vikeClientDirectory)) fail("Vike client build is missing.");
...
if (!existsSync(legacyAssetsDirectory)) fail("Historical SPA assets are missing.");
for (const source of walkFiles(legacyAssetsDirectory)) {
  copyWithCollisionCheck(
    source,
    join(temporaryDirectory, "assets", relative(legacyAssetsDirectory, source)),
  );
}
...
promoteOutput();
console.log(
  `[publish-static-output] Validated and promoted ${prerenderedDocuments.length} prerendered pages and the SPA fallback.`,
);