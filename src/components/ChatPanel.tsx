// ... (imports identiques)
  ...
  const startConversation = async () => {
    ...
    setIsLoading(true);
    try {
      // C'EST ICI QUE TON NOUVEAU TEXTE APPARAÎT (DÉBUT DU CHAT)
      const initialPrompt = `Tu es Édouard. Tu dois répondre EXACTEMENT ceci, avec un saut de ligne entre chaque option :
...
// ... (logique handleSend identique)

  return (
    <div className="flex flex-col h-screen bg-background relative overflow-hidden">
      {/* RESET SESSION EN HAUT À GAUCHE */}
      <div className="fixed top-2 left-2 z-[100] opacity-50 hover:opacity-100 transition-opacity">
        <button onClick={handleForceSignOut} className="flex items-center gap-1.5 bg-red-950/40 hover:bg-red-800 text-red-100 px-2 py-1.5 rounded-md text-[9px] font-bold uppercase border border-red-500/20">
      ...
      <div className="flex-1 flex flex-col items-center justify-start p-6 bg-[#0B0E14] text-slate-200 overflow-y-auto">
        ...
        {!disclaimerAccepted && displayMessages.length === 0 ? (
          /* RETOUR AU TEXTE ORIGINAL DE L'IMAGE POUR LE POP-UP */
          <div className="max-w-2xl w-full bg-[#161B22] border border-slate-800 rounded-3xl p-10 shadow-2xl space-y-8 my-12 animate-in fade-in zoom-in duration-500">
            ...
            <div className="space-y-1.5">
              <h1 className="text-4xl font-bold text-white tracking-tight">Je suis <span className="text-indigo-500">Édouard.</span></h1>
            ...
            <div className="space-y-6 text-slate-300 leading-relaxed text-[15px]">
              ...
              <p>
                Je m'exprime de manière <span className="font-semibold text-white">ferme, assertive et juste</span>, ne le prends pas pour toi. 
              ...
              <div className="pl-4 border-l-2 border-indigo-500/40 py-1 text-slate-400 text-[13px] italic bg-indigo-500/5">
                J'utilise uniquement des données réelles et vérifiables issues du web. Je n'invente jamais de chiffres, de marché ou de tendances. Si une information fiable n'est pas disponible, je le dis clairement.
              </div>
            </div>

            {/* AVERTISSEMENT */}
            <div onClick={() => setIsChecked(!isChecked)} className={cn("p-6 rounded-2xl border transition-all cursor-pointer flex gap-4 bg-[#0B0E14]", isChecked ? "border-indigo-500/50" : "border-slate-800/60")}>
              <div className={cn("mt-1 w-5 h-5 rounded border flex items-center justify-center shrink-0 transition-colors bg-black/20", isChecked ? "bg-indigo-600 border-indigo-600 text-white" : "border-slate-700")}>
                {isChecked && <Check size={12} />}
              </div>
              <p className="text-[11px] text-slate-400 leading-snug">
                <span className="font-bold text-slate-300">AVERTISSEMENT :</span> Les analyses sont fournies à titre informatif et consultatif uniquement. Elles ne constituent pas une garantie de résultat ni un conseil engageant. L'utilisation des informations et les décisions prises relèvent entièrement de la responsabilité de l'utilisateur.
              </p>
            </div>

            <button disabled={!isChecked || isLoading} onClick={startConversation} className="w-full py-4 bg-gradient-to-r from-indigo-600 to-violet-700 text-white rounded-2xl font-bold flex items-center justify-center gap-2 active:scale-95 disabled:opacity-30">
              {isLoading ? "Initialisation..." : "Commencer l'analyse"} <ArrowRight size={20} />
            </button>
          </div>
        ) : (
          /* ZONE DE CHAT - Avec le nouveau message d'Édouard */
          <div className="max-w-2xl w-full flex-1 flex flex-col min-h-0 pb-24">
            <div className="flex-1 space-y-6">
              {displayMessages.map((msg: any, i: number) => (
                <div key={i} className={cn("flex flex-col animate-in fade-in", msg.role === "user" ? "items-end" : "items-start")}>
                  <div className={cn("flex gap-3 max-w-[85%]", msg.role === "user" ? "flex-row-reverse" : "")}>
                    <div className={cn("w-8 h-8 rounded-lg flex items-center justify-center shrink-0", msg.role === "assistant" ? "bg-indigo-600 text-white" : "bg-muted")}>
                      {msg.role === "assistant" ? <Brain size={18} /> : <User size={18} />}
                    </div>
                    <div className={cn("rounded-2xl px-4 py-3 text-sm border shadow-sm", msg.role === "assistant" ? "bg-[#161B22] border-slate-800" : "bg-primary/5")}>
              ...
              {isLoading && <div className="text-xs animate-pulse ml-11 text-indigo-500">Édouard analyse...</div>}
