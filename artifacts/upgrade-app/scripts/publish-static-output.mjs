...
function copyWithCollisionCheck(source, destination) {
  if (existsSync(destination)) {
    if (digest(source) !== digest(destination)) {
      fail(`Asset collision with different content: ${relative(publicDirectory, destination)}`);
    }
    return;
  }
...
cpSync(vikeClientDirectory, publicDirectory, { recursive: true });
...
copyFileSync(
  ...
  join(publicDirectory, "legacy-spa.html"),
);

for (const document of prerenderedDocuments) {
  assertFile(publicDirectory, document);
}

for (const pageContext of pageContexts) {
  assertFile(publicDirectory, pageContext);
}

for (const publicFile of mandatoryPublicFiles) {
  assertFile(publicDirectory, publicFile);
}

assertFile(publicDirectory, "legacy-spa.html");

const htmlFiles = walkFiles(publicDirectory).filter((filePath) =>
...
for (const htmlFile of htmlFiles) {
  ...
  for (const assetReference of localAssetReferences) {
    assertFile(publicDirectory, assetReference.slice(1));
  }
}

console.log(
  `[publish-static-output] Published ${prerenderedDocuments.length} prerendered HTML documents, the historical SPA fallback, and all required public files.`,
);